...
const postController = {
  createPost: grpcLoggerMiddleware(async (call, callback) => {
    try {
      const { postName, userId, address, education, description, productId, customerId } = call.request;
      const post = new Post({
        postName,
  ...
  getPost: grpcLoggerMiddleware(async (call, callback) => {
    try {
      const { id } = call.request;
  ...
  updatePost: grpcLoggerMiddleware(async (call, callback) => {
    try {
      const { id, postName, userId, address, education, description, productId, customerId } = call.request;
      const post = await Post.findByIdAndUpdate(id, {
        postName,
  ...
  deletePost: grpcLoggerMiddleware(async (call, callback) => {
    try {
      const { id } = call.request;
      const post = await Post.findByIdAndDelete(id);
      if (!post) {
  ...
  listPosts: grpcLoggerMiddleware(async (call, callback) => {
    try {
      const { page, limit } = call.request;
      const pageNum = parseInt(page, 10) || 1;
      const limitNum = parseInt(limit, 10) || 10;
  ...
  searchPosts: grpcLoggerMiddleware(async (call, callback) => {
    try {
      const { postName, userId, page, limit } = call.request;
      const pageNum = parseInt(page, 10) || 1;
      const limitNum = parseInt(limit, 10) || 10;
